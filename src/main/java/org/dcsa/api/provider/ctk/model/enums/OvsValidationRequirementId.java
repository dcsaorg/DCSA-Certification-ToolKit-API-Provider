...
public enum OvsValidationRequirementId {
    OVS_REQUIREMENT_ID_1("OVS.REQUIREMENT.ID.1", "Major Version number must be present in URL"),
    OVS_REQUIREMENT_ID_2("OVS.REQUIREMENT.ID.2", "Full Version number present in response headers"),
    OVS_REQUIREMENT_ID_3("OVS.REQUIREMENT.ID.3", "HTTP verbs (GET, PUT, PATCH, POST, DELETE, HEAD) must be in request"),
    OVS_REQUIREMENT_ID_4("OVS.REQUIREMENT.ID.4", "URL must adhere to Kebab-case"),
    OVS_REQUIREMENT_ID_5("OVS.REQUIREMENT.ID.5", "Content type headers must be application/json"),
    OVS_REQUIREMENT_ID_6("OVS.REQUIREMENT.ID.6", "Links to current, previous, next, first and last page SHOULD be available in the response headers"),
    OVS_REQUIREMENT_ID_7("OVS.REQUIREMENT.ID.7", "Encoding of API must be in UTF8 format"),
    OVS_REQUIREMENT_ID_8("OVS.REQUIREMENT.ID.8", "GET /event-subscriptions returns all subscriptions without any query parameter"),
    ...
    private static final Map<String, OvsValidationRequirementId> RequirementId = new HashMap<>();

    private final String id;
    private final String details;

    OvsValidationRequirementId(String id, String details) {
        this.id = id;
    ...
    public static OvsValidationRequirementId getById(String id){
